...
$(document).ready(function () {
    ...
    // Affichage de PDF à l'ouvertur de popup dans la liste des types d'offre
    $(document).on('click', '.wrapper-box-image-template-liste', function () {
        $('#popup-show-type-offre').show();
        const typeSelected = listeType2.filter(t => {
            return t.typ_id == $(this).attr('data-type-id')
        })
        $('#text-condition-offre').html(typeSelected[0].typ_condition)
        $('#text-description-offre').html(typeSelected[0].typ_description)
